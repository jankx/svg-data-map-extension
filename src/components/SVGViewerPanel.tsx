...
export function SVGViewerPanel({
  ...
  return (
    <div id="viewer-root-panel" className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
      {/* 1. Left Map side */}
        ...
        <div className="bg-white border border-indigo-50/70 rounded-xl p-4 shadow-sm flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          ...
          <div className="flex items-center gap-1.5 self-start sm:self-center">
            ...
            <button
              ...
              className={`p-1.5 px-3 rounded-lg text-xs font-semibold cursor-pointer transition ${
                config.title.includes('Việt Nam') 
            ...
            <button
              ...
              className={`p-1.5 px-3 rounded-lg text-xs font-semibold cursor-pointer transition ${
                config.title.includes('Triển Lãm') 
                  ? 'bg-indigo-600 text-white shadow-md shadow-indigo-500/10' 
                  : 'bg-slate-100 hover:bg-slate-200 text-slate-700'
              }`}
            >
      ...
      {/* 2. Right Content Block layout - EXACTLY styled like in user's request screenshot! */}
      <div 
        id="viewer-sidebar-root"
        className="lg:col-span-5 xl:col-span-4 rounded-2xl p-6 flex flex-col justify-between shadow-md border overflow-hidden min-h-[550px] transition-all"
        style={{
          backgroundColor: '#d2ebfa', // Sky pastel background as shown in the screenshot
          borderColor: '#bce1f7'
        }}
      >
        {activeRegion ? (
              ...
              {activeRegion.items && activeRegion.items.length > 0 ? (
                activeRegion.items.map((item, idx) => (
                  ...
                  >
                    ...
                    {/* Standard hyperlink to let users explore real resources cleanly */}
                    <a
                      href={item.linkUrl || '#'}
