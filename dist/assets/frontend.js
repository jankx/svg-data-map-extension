(()=>{document.addEventListener("DOMContentLoaded",()=>{let E=document.querySelectorAll(".jankx-svg-data-map-runtime"),S=document.querySelectorAll(".jankx-svg-data-map-info-runtime"),k=new Map;S.forEach(o=>{let g=o.getAttribute("data-map-id")||"default-map",x=o.querySelector(".jankx-svg-map-info-content");x&&k.set(g,x)}),E.forEach(o=>{let g=o.getAttribute("data-config"),x=o.getAttribute("data-map-id")||"default-map";if(!g)return;let y;try{y=JSON.parse(g)}catch{return}let w=y.regions||[],d=o.querySelector(".jankx-svg-map-wrapper svg"),c=o.querySelector(".jankx-svg-map-info-panel")||k.get(x);if(!d||!c)return;let h=new Map;w.forEach(n=>h.set(n.id,n));let v=!1,m=null,L=()=>{c&&(c.innerHTML=`
            ...
            `)},j=async n=>{if(v||m===n)return;let e=h.get(n);if(!e)return;if(m){let a=h.get(m);a&&a.pathIds&&a.pathIds.forEach(f=>{let p=d.querySelector(`#${f}`);p&&p.classList.remove("jankx-map-active")});let r=o.querySelector(`.jankx-marker-btn[data-region-id="${m}"]`);r&&r.classList.remove("jankx-map-active")}m=n,e.pathIds&&e.pathIds.forEach(a=>{let r=d.querySelector(`#${a}`);r&&r.classList.add("jankx-map-active")});let t=o.querySelector(`.jankx-marker-btn[data-region-id="${n}"]`);t&&t.classList.add("jankx-map-active");let i=o.querySelector(".jankx-map-active-title")||(c.parentElement?c.parentElement.querySelector(".jankx-map-active-title"):null);i&&(i.textContent=e.name||e.label||"Khu v\u1EF1c");let l="";e.items&&e.items.length>0&&e.items.forEach(a=>{l+=`
                ...
                `};if(!e.termId){console.log("SVG Map: No termId, showing manual data only"),s();return}L(),v=!0;try{let a=window.jankxViewsData?.ajaxUrl||"/wp-admin/admin-ajax.php",r=new FormData;r.append("action","svg_data_map_fetch_posts"),r.append("term_id",e.termId),r.append("taxonomy",e.taxonomy||"category"),r.append("post_type",e.postType||"post");let p=await(await fetch(a,{method:"POST",body:r})).json();p.success?s(p.data.html):b()}catch(a){console.error("SVG Map: AJAX error:",a),b()}finally{v=!1}},u=(n,e)=>{let t=h.get(n);if(!t)return;t.pathIds&&t.pathIds.forEach(l=>{let s=d.querySelector(`#${l}`);s&&(e?s.classList.add("jankx-map-hover"):s.classList.remove("jankx-map-hover"))});let i=o.querySelector(`.jankx-marker-btn[data-region-id="${n}"]`);i&&(e?i.classList.add("jankx-map-marker-hover"):i.classList.remove("jankx-map-marker-hover"))};w.forEach(n=>{n.pathIds&&n.pathIds.forEach(e=>{let t=d.querySelector(`#${e}`);t&&(t.classList.add("jankx-map-region-clickable"),t.addEventListener("click",i=>{i.preventDefault(),j(n.id)}),t.addEventListener("mouseenter",()=>u(n.id,!0)),t.addEventListener("mouseleave",()=>u(n.id,!1)))})}),o.querySelectorAll(".jankx-marker-btn").forEach(n=>{let e=n,t=e.getAttribute("data-region-id"),i=e.getAttribute("data-path-id");if(i){let l=d.querySelector(`#${i}`);if(l&&typeof l.getBBox=="function")try{let s=l.getBBox(),a=s.x+s.width/2,r=s.y+s.height/2,f=d.getAttribute("viewBox")?.split(" ").map(Number)||[0,0,1e3,1e3],p=f[2],$=f[3],M=a/p*100,q=r/$*100;e.style.left=`${M}%`,e.style.top=`${q}%`,e.style.display="flex",e.style.position="absolute",e.style.transform="translate(-50%, -50%)"}catch(s){console.error("SVG Map: Failed to calculate bbox for",i,s)}}e.addEventListener("click",l=>{l.preventDefault(),l.stopPropagation(),console.log("SVG Map: Marker clicked for region:",t),t&&j(t)}),e.addEventListener("mouseenter",()=>{t&&u(t,!0)}),e.addEventListener("mouseleave",()=>{t&&u(t,!1)})})})});})();
