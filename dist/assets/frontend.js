(()=>{document.addEventListener("DOMContentLoaded",()=>{let I=document.querySelectorAll(".jankx-svg-data-map-runtime"),A=document.querySelectorAll(".jankx-svg-data-map-info-runtime"),w=new Map;A.forEach(s=>{let h=s.getAttribute("data-map-id")||"default-map",u=s.querySelector(".jankx-svg-map-info-content");u&&w.set(h,u)}),I.forEach(s=>{let h=s.getAttribute("data-config"),u=s.getAttribute("data-map-id")||"default-map";if(!h)return;let b;try{b=JSON.parse(h)}catch{return}let E=b.regions||[],m=s.querySelector(".jankx-svg-map-wrapper svg"),d=s.querySelector(".jankx-svg-map-info-panel")||w.get(u);if(!m||!d)return;let g=new Map;E.forEach(e=>g.set(e.id,e));let v=!1,x=null,D=()=>{d&&(d.innerHTML=`
            ...
            `)},S=async e=>{if(v||x===e)return;let t=g.get(e);if(!t)return;if(x){let a=g.get(x);a&&a.pathIds&&a.pathIds.forEach(c=>{let f=m.querySelector(`#${c}`);f&&f.classList.remove("jankx-map-active")});let o=s.querySelector(`.jankx-marker-btn[data-region-id="${x}"]`);o&&o.classList.remove("jankx-map-active")}x=e,t.pathIds&&t.pathIds.forEach(a=>{let o=m.querySelector(`#${a}`);o&&o.classList.add("jankx-map-active")});let n=s.querySelector(`.jankx-marker-btn[data-region-id="${e}"]`);n&&n.classList.add("jankx-map-active");let r=s.querySelector(".jankx-map-active-title")||(d.parentElement?d.parentElement.querySelector(".jankx-map-active-title"):null);r&&(r.textContent=t.name||t.label||"Khu v\u1EF1c");let i="";t.items&&t.items.length>0&&t.items.forEach(a=>{i+=`
                ...
                `};if(!t.termId){console.log("SVG Map: No termId, showing manual data only"),l();return}D(),v=!0;try{let a=window.jankxViewsData?.ajaxUrl||"/wp-admin/admin-ajax.php",o=new FormData;o.append("action","svg_data_map_fetch_posts"),o.append("term_id",t.termId),o.append("taxonomy",t.taxonomy||"category"),o.append("post_type",t.postType||"post");let f=await(await fetch(a,{method:"POST",body:o})).json();f.success?l(f.data.html):j()}catch(a){console.error("SVG Map: AJAX error:",a),j()}finally{v=!1}},y=(e,t)=>{let n=g.get(e);if(!n)return;n.pathIds&&n.pathIds.forEach(i=>{let l=m.querySelector(`#${i}`);l&&(t?l.classList.add("jankx-map-hover"):l.classList.remove("jankx-map-hover"))});let r=s.querySelector(`.jankx-marker-btn[data-region-id="${e}"]`);r&&(t?r.classList.add("jankx-map-marker-hover"):r.classList.remove("jankx-map-marker-hover"))};E.forEach(e=>{e.pathIds&&e.pathIds.forEach(t=>{let n=m.querySelector(`#${t}`);n&&(n.classList.add("jankx-map-region-clickable"),n.addEventListener("click",r=>{r.preventDefault(),S(e.id)}),n.addEventListener("mouseenter",()=>y(e.id,!0)),n.addEventListener("mouseleave",()=>y(e.id,!1)))})});let p=1,L=Array.from(s.querySelectorAll(".jankx-marker-btn")),G=()=>{L.forEach(e=>{e.style.transform=`translate(-50%, -50%) scale(${p})`,e.style.transformOrigin="center bottom"})};L.forEach(e=>{let t=e.getAttribute("data-region-id"),n=e.getAttribute("data-path-id"),r=(i=5)=>{if(n){let l=m,a=l.querySelector(`#${n}`),o=s.querySelector(".jankx-markers-layer");if(a&&o&&typeof a.getBBox=="function")try{let c=a.getBBox(),f=c.x+c.width/2,V=c.y+c.height/2,M=l.getScreenCTM();if(M){let k=l.createSVGPoint();k.x=f,k.y=V;let $=k.matrixTransform(M),q=o.getBoundingClientRect(),H=($.x-q.left)/p,C=($.y-q.top)/p,T=g.get(t),O=(T?.marker?.markerOffsetX??0)/p,P=(T?.marker?.markerOffsetY??0)/p;e.style.left=`${H+O}px`,e.style.top=`${C+P}px`,e.style.transform=`translate(-50%, -50%) scale(${p})`,e.style.transformOrigin="center bottom",e.style.display="flex",e.style.position="absolute"}else i>0&&setTimeout(()=>r(i-1),100)}catch(c){console.error("SVG Map: Failed to calculate bbox for",n,c)}else i>0&&setTimeout(()=>r(i-1),100)}};r(),window.addEventListener("resize",()=>{r()}),e.addEventListener("click",i=>{i.preventDefault(),i.stopPropagation(),t&&S(t)}),e.addEventListener("mouseenter",()=>{t&&y(t,!0)}),e.addEventListener("mouseleave",()=>{t&&y(t,!1)})}),(s.querySelector("#map-container-root")||s).addEventListener("wheel",e=>{e.preventDefault();let t=e.deltaY<0?.05:-.05;p=Math.max(.5,Math.min(5,p+t)),G()},{passive:!1})})});})();
