(()=>{document.addEventListener("DOMContentLoaded",()=>{let I=document.querySelectorAll(".jankx-svg-data-map-runtime"),A=document.querySelectorAll(".jankx-svg-data-map-info-runtime"),w=new Map;A.forEach(s=>{let h=s.getAttribute("data-map-id")||"default-map",u=s.querySelector(".jankx-svg-map-info-content");u&&w.set(h,u)}),I.forEach(s=>{let h=s.getAttribute("data-config"),u=s.getAttribute("data-map-id")||"default-map";if(!h)return;let b;try{b=JSON.parse(h)}catch{return}let E=b.regions||[],m=s.querySelector(".jankx-svg-map-wrapper svg"),c=s.querySelector(".jankx-svg-map-info-panel")||w.get(u);if(!m||!c)return;let g=new Map;E.forEach(e=>g.set(e.id,e));let v=!1,x=null,D=()=>{c&&(c.innerHTML=`
            ...
            `)},S=async e=>{if(v||x===e)return;let t=g.get(e);if(!t)return;if(x){let a=g.get(x);a&&a.pathIds&&a.pathIds.forEach(p=>{let f=m.querySelector(`#${p}`);f&&f.classList.remove("jankx-map-active")});let o=s.querySelector(`.jankx-marker-btn[data-region-id="${x}"]`);o&&o.classList.remove("jankx-map-active")}x=e,t.pathIds&&t.pathIds.forEach(a=>{let o=m.querySelector(`#${a}`);o&&o.classList.add("jankx-map-active")});let n=s.querySelector(`.jankx-marker-btn[data-region-id="${e}"]`);n&&n.classList.add("jankx-map-active");let i=s.querySelector(".jankx-map-active-title")||(c.parentElement?c.parentElement.querySelector(".jankx-map-active-title"):null);i&&(i.textContent=t.name||t.label||"Khu v\u1EF1c");let r="";t.items&&t.items.length>0&&t.items.forEach(a=>{r+=`
                ...
                `};if(!t.termId){console.log("SVG Map: No termId, showing manual data only"),l();return}D(),v=!0;try{let a=window.jankxViewsData?.ajaxUrl||"/wp-admin/admin-ajax.php",o=new FormData;o.append("action","svg_data_map_fetch_posts"),o.append("term_id",t.termId),o.append("taxonomy",t.taxonomy||"category"),o.append("post_type",t.postType||"post");let f=await(await fetch(a,{method:"POST",body:o})).json();f.success?l(f.data.html):j()}catch(a){console.error("SVG Map: AJAX error:",a),j()}finally{v=!1}},y=(e,t)=>{let n=g.get(e);if(!n)return;n.pathIds&&n.pathIds.forEach(r=>{let l=m.querySelector(`#${r}`);l&&(t?l.classList.add("jankx-map-hover"):l.classList.remove("jankx-map-hover"))});let i=s.querySelector(`.jankx-marker-btn[data-region-id="${e}"]`);i&&(t?i.classList.add("jankx-map-marker-hover"):i.classList.remove("jankx-map-marker-hover"))};E.forEach(e=>{e.pathIds&&e.pathIds.forEach(t=>{let n=m.querySelector(`#${t}`);n&&(n.classList.add("jankx-map-region-clickable"),n.addEventListener("click",i=>{i.preventDefault(),S(e.id)}),n.addEventListener("mouseenter",()=>y(e.id,!0)),n.addEventListener("mouseleave",()=>y(e.id,!1)))})});let d=1,L=Array.from(s.querySelectorAll(".jankx-marker-btn")),G=()=>{L.forEach(e=>{e.style.transform=`translate(-50%, -50%) scale(${d})`,e.style.transformOrigin="center bottom"})};L.forEach(e=>{let t=e.getAttribute("data-region-id"),n=e.getAttribute("data-path-id"),i=()=>{if(n){let r=m,l=r.querySelector(`#${n}`),a=s.querySelector(".jankx-markers-layer");if(l&&a&&typeof l.getBBox=="function")try{let o=l.closest("g")||r,p=l.getBBox(),f=p.x+p.width/2,V=p.y+p.height/2,M=o.getCTM()||r.getScreenCTM();if(M){let k=r.createSVGPoint();k.x=f,k.y=V;let $=k.matrixTransform(M),q=a.getBoundingClientRect(),C=($.x-q.left)/d,H=($.y-q.top)/d,T=g.get(t),O=(T?.marker?.markerOffsetX??0)/d,P=(T?.marker?.markerOffsetY??0)/d;e.style.left=`${C+O}px`,e.style.top=`${H+P}px`,e.style.transform="translate(-50%, -50%)",e.style.transformOrigin="center bottom",e.style.display="flex",e.style.position="absolute"}}catch(o){console.error("SVG Map: Failed to calculate bbox for",n,o)}}};i(),window.addEventListener("resize",()=>{i()}),e.addEventListener("click",r=>{r.preventDefault(),r.stopPropagation(),t&&S(t)}),e.addEventListener("mouseenter",()=>{t&&y(t,!0)}),e.addEventListener("mouseleave",()=>{t&&y(t,!1)})}),(s.querySelector("#map-container-root")||s).addEventListener("wheel",e=>{e.preventDefault();let t=e.deltaY<0?.05:-.05;d=Math.max(.5,Math.min(5,d+t)),G()},{passive:!1})})});})();
